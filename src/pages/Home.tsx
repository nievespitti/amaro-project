    ...
    >
      ...
      <div className="mb-16">
        <div className="rounded-2xl overflow-hidden">
          <img
            src={isMobile ? "/src/assets/images/GIF-Home-Vertical.gif" : "/src/assets/images/GIF-Home-Horizontal.gif"}
      ...
      <div id="SobreNosotros" className="mt-0 mb-8 w-[70%] space-y-2">
        <p className="sm:text-small md:text-medium lg:text-[38px] leading-tight text-[#090F4C]">
          ¡Hola! Somos Estudio Amaro y hacemos branding y diseño. Nos dedicamos a acompañar e impulsar marcas desde la
          sencillez y la honestidad.
        </p>
      </div>
      <div className="mt-0 mb-16 w-[70%]">
        <p className="sm:text-small md:text-medium lg:text-[38px] leading-tight text-[#090F4C]">
          Nos gusta cuidar cada detalle. Trabajamos mano a mano con nuestros clientes adaptándonos a sus necesidades
        ...
        {projects.map((project) => (
          <Link key={project.id} to={project.path}>
            <motion.div whileHover={{ scale: 1.02 }} className="overflow-hidden">
              <picture>
                <source media="(min-width: 768px)" srcSet={project.image} type="image/jpeg" />
                <source media="(max-width: 767px)" srcSet={project.imageMobile} type="image/jpeg" />
                <LazyLoadImage
                  src={isMobile ? project.imageMobile : project.image}
                  alt={project.title}
                  effect="blur"
                  className={`w-full ${isMobile ? "aspect-[3/4]" : "aspect-[3/2]"} object-cover rounded-xl block`}
                  wrapperClassName="block"
                />
              </picture>
              <p className="text-[18px] md:text-[28px] lg:text-[28px] mt-0">{project.title}</p>
            </motion.div>
          </Link>
        ))}
      </div>
      ...
      {/* Sección de contacto */}
      <section id="Contacto" className="mt-16 mb-16 w-[85%]" style={{ scrollMarginTop: "100px" }}>
        <div className="space-y-8">
          ...
          <div className="space-y-8">
            <p className="sm:text-[28px] md:text-[38px] lg:text-[38px] leading-tight custom-font text-[#090F4C]">
              Puedes escribir a paula@estudioamaro.com <br />o llamarnos al +34 672 048 172.
            </p>
          </div>
          <div className="space-y-8">
            <p className="sm:text-[28px] md:text-[38px] lg:text-[38px] leading-tight custom-font text-[#090F4C]">
              Si lo prefieres, podemos reunirnos en: C/ Silva, 2 4º 28013 Madrid
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-6 mt-8">
          ...
          >
            <img src={InstagramIcon || "/placeholder.svg"} alt="Instagram" width="48" height="48" />
          </a>
          <a
            ...
            className="text-[#090F4C] hover:opacity-70 transition-opacity"
          >
            <img src={LinkedInIcon || "/placeholder.svg"} alt="LinkedIn" width="48" height="48" />
          </a>
          <a
            ...
            className="text-[#090F4C] hover:opacity-70 transition-opacity"
          >
            <img src={SpotifyIcon || "/placeholder.svg"} alt="Spotify" width="48" height="48" />
          </a>
        </div>
      </section>
      </div>
